...
> *"主进程不跑 agent, Sidecar 来跑"* — JSON-RPC over Unix Socket, RingBuffer 8MB。
>
...
| **RingBuffer** | 8MB 环形缓冲区，捕获 stdout/stderr | Sidecar 内部 |
...
### RingBuffer — 8MB 环形缓冲区

Sidecar 捕获所有子进程的 stdout/stderr，存入一个 8MB 的环形缓冲区。满了就覆盖最旧的数据——保留最近的日志，不无限增长。
...
class SidecarServer {
    constructor() {
        this.ringBuffer = new RingBuffer(8 * 1024 * 1024); // 8MB
...
```javascript
// index.js — Main Process 启动 Sidecar
const { spawn } = require('child_process');
const sidecarEntry = require.resolve(
    'agent-cli/sidecar-entry',
    { paths: [app.getAppPath() + '/CLI runtime resources'] }
);
